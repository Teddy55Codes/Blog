...
linePlot <- function(sortedData, framework)
  ggplot(sortedData, aes(x = Iterations, y = Value, group = Target_Method)) +
    ...
    ggtitle(paste("Primes Benchmark Results ", framework)) +
...
linePlotForAllFrameworks <- function(sortedData, framework, showLegend)
  ggplot(sortedData, aes(x = Iterations, y = Value, group = Target_Method)) +
    ...
    theme(plot.margin = unit(c(1.4, 0.2, 0, 0.2), units = "in"),
          ...
          legend.box.margin = margin(-560, 0, 0, -110),
...
for (file in files) {
  ...
  for (framework in names(splitLists)) {
    ...
    benchmarkName <- paste0("benchmark_Primes_", frameworkFileName, ".png")
  ...
  benchmarkName <- "benchmark_Primes_AllFrameworks.png"
  save_plot(
    benchmarkName,
    do.call(
      plot_grid,
      args = list(
        ...
        nrow = 1,
        labels = "Results for Primes Benchmark",
        label_size = 20,
        label_x = -0.21,
        label_y = 0.95)),
    base_height = 8,
